...
import { fetchSupply } from '../../../api/supplyApi';
...
export default function Supply() {
    ...
    const columns = [
        {
            ...
            renderCell: ({ row }) => (
                <Card sx={{ height: '100%', width: '100%', p: .2 }}>
                    {row?.itemId?.image ? (
                        <img style={{ width: '100%', height: '100%' }} src={row.itemId.image} />
                    ) : (
        ...
        },
        {
            ...
            editable: true,
            flex: 1,
            renderCell: ({ row }) => row?.itemId?.item ? row.itemId.item : 'Null'
        },
        {
            field: 'quantity',
            type: 'number',
    ...
    const handleGetData = async () => {
        const { data, error } = await fetchSupply();
        if (error) {
            console.log(error)
        } else {
...
function StoreContent({ handleGetData }) {
    ...
    return (
        <>
            <Button endIcon={<Add />} onClick={() => setModal(true)}>Add Supply</Button>
            <Drawer
                open={modal}
                anchor='right'
                sx={{ zIndex: 1300 }}
